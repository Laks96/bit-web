const data = (function () {
    class Show {
        constructor(name, image, url, average, id, seasons, cast, summary) {
            this.name = name;
            this.image = image;
            this.url = url;
    ...
    class Season {
        constructor(id, premiereDate, endDate) {
            this.id = id;
    ...
    function fetchShow(showId, successSeasons) {
        ...
        $.get(urlSingleShow, function (apiShow) {
            ...
            for (let i = 0; i < apiSeasons.length; i++) {
                ...
                const season = new Season(apiSeason.id, apiSeason.premiereDate, apiSeason.endDate, apiSeason.summary)
                mySeasons.push(season)

            }

            const show = new Show(apiShow.name, apiShow.image.medium, apiShow.url, apiShow.rating.average, apiShow.id, mySeasons, cast, apiShow.summary);
